...
import { deleteTeacherPostsById, getTeacherPosts, UpdateCourseStatus } from "./TeacherPosts.service";
...
export default function TeacherPostsPage({ }: Props): ReactElement {
    ...
    const onDeleteClick = async (item: any) => {
        let res = await deleteTeacherPostsById(item.id)
    ...
    const onOpenClick = async (item: any) => {
        history.push(`/me/teacher-post/${item.id}`)
    }
    ...
    const loadData = async (queries: any) => {
        ...
        let res = await getTeacherPosts(queries)
        if (res.status === 200 || res.status === 201) {
            let data = await res.json()
                ...
                {response.data?.map((item: any, index: number) => (
                    <Col key={index} className="mt-3" sm="6">
                        <Card className="shadow-lg bg-white  rounded" role="button">
                            <Card.Body style={{ backgroundColor: '#F1EDED' }}>
                                ...
                                <Row>
                                    <Col>
                                        ...
                                        <Row className="text-capitalize mt-1">
                                            <Col lg={6} sm={12}><strong>Area Covered :</strong> {item.area_covered}</Col>
                                        </Row>
                                        <Row className="text-capitalize mt-1">
