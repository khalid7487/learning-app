...
import {create, deleteItemById, download, getPaginate, update} from "./script.service";
import EditModal from "./Edit.modal";
import CreateModal from "./Create.modal";
import {ToastFailedMsg, ToastSuccessMsg, ToastWarningMsg} from "../../../common/toast";
import {useHistory} from "react-router-dom";
import data from '../../../common/mock-data.json';
...
export default function ScriptPaginationPage({}: Props): ReactElement {
    ...
    const loadData = async () => {
        let res = await getPaginate("LET", 1, 5)
        if (res.status === 200 || res.status === 201) {
            let {data} = await res.json()
            setResponse(data)
        } else {
