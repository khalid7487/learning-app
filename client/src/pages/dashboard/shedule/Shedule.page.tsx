...
import { create, deleteItemById, download, gets, update, UpdateScheduleStatus } from "./Shedule.service";
...
export default function TripPage({ }: Props): ReactElement {
    ...
    const onOpenClick = async (item: any) => {
        history.push(`/me/shedule/${item.id}`)
    }
    ...
    const approveStatus = async (item: any) => {
        ...
        if (res.status === 200 || res.status === 201) {
            let data = await res.json()
            await loadData({});
            ToastSuccessMsg("Schedule Approved Successfully");
            // if (isMounted) setResponse(data)
            // setResponse(data)
        } else {
            //let error = await res.json()
            ToastFailedMsg("Schedule Approved faield ");
        }
    }
    ...
    const rejectedStatus = async (item: any) => {
        ...
        if (res.status === 200 || res.status === 201) {
            let data = await res.json()
            await loadData({});
            ToastSuccessMsg("Schedule Rejected Successfully");
        } else {
            //let error = await res.json()
            ToastFailedMsg("Schedule Rejected faield ");
        }
    }
                            ...
                            <Card.Body>
                                <Row>
                                    <Col>
                                        ...
                                        <Row className="mt-1">
                                            <Col lg={4} sm={12} ><strong className="text-capitalize ">Schedule Type:</strong>
                                                {item?.schedule_type === 0 && (<span> Free</span>)}
                                                {item?.schedule_type === 1 && (<span> Paid</span>)}
                                            </Col>
                                            <Col lg={4} sm={12} className="text-capitalize "><strong>Schedule Name:</strong> {item?.name} </Col>
                                            <Col lg={4} sm={12} className="text-capitalize "><strong>Duration:</strong> {item.duration} <strong className="text-warning"> Hour</strong></Col>
                                        </Row>

                                        <Row className="text-capitalize mt-1">
                                            <Col lg={4} sm={12} className=" font-weight-bold">Shedule Status:
                                                {item.status === 0 && (<span style={{ color: 'blue' }} > Pending</span>)}
                                                {item.status === 1 && (<span style={{ color: 'green' }}> Active</span>)}
                                                {item.status === 5 && (<span style={{ color: 'red' }}> InActive</span>)}
                                            </Col>
                                            <Col lg={4} sm={12} className=" text-capitalize"><strong>Location:</strong> {item.location}</Col>
                                            <Col lg={4} sm={12} className=" text-capitalize"><strong>Meeting Url:</strong> {item.meeting_url}</Col>
                                        </Row>

                                        <Row className="mt-1">
                                            <Col lg={4} sm={12} className=" text-capitalize "><strong>Start Date: </strong> {new Date(item.start).toLocaleString()}</Col>
                                            <Col lg={4} sm={12} className=" text-capitalize "><strong>End Date: </strong> {new Date(item.end).toLocaleString()}</Col>
                                        </Row>
                                        <Row>
